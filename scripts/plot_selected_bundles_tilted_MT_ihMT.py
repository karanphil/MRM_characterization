...
def main():
    ...
    for i in range(nb_bundles):
        ...
        if bundles_names[i] in extracted_bundles:
            ...
            ax[row, col + 2].axhline(y=mean_ihmtr, color=cm.naviaS(5), linewidth=1, alpha=0.5, linestyle="-")
            mean_ihmtsat = np.sum(result['ihMTsat'][is_not_nan] * result['Nb_voxels'][is_not_nan] / np.sum(result['Nb_voxels'][is_not_nan]))
            ax[row, col + 3].axhline(y=mean_ihmtsat, color=cm.naviaS(5), linewidth=1, alpha=0.5, linestyle="-")
            ...
            # ax[row, col + 2].plot(highres_bins, BSpline(*ihmtr_fit)(highres_bins), "--", color=cm.naviaS(5))
            ...
            ax[row, col + 2].set_ylim(0.975 * np.min((np.nanmin(result['ihMTR'][is_measures]), np.nanmin(result_tilted['ihMTR'][is_measures]))),
                                      1.025 * np.max((np.nanmax(result['ihMTR'][is_measures]), np.nanmax(result_tilted['ihMTR'][is_measures]))))
            ax[row, col + 2].set_yticks([np.round(np.min((np.nanmin(result['ihMTR'][is_measures]), np.nanmin(result_tilted['ihMTR'][is_measures]))), decimals=1),
            ...
            ax[row, col + 3].set_ylim(0.975 * np.min((np.nanmin(result['ihMTsat'][is_measures]), np.nanmin(result_tilted['ihMTsat'][is_measures]))),
                                      1.025 * np.max((np.nanmax(result['ihMTsat'][is_measures]), np.nanmax(result_tilted['ihMTsat'][is_measures]))))
            ax[row, col + 3].set_yticks([np.round(np.min((np.nanmin(result['ihMTsat'][is_measures]), np.nanmin(result_tilted['ihMTsat'][is_measures]))), decimals=1),
            ...
            ax[row, col + 1].set_xlim(0, 90)
            # ax[row, col + 1].tick_params(axis='y', labelcolor="C2")
            ax[row, col + 2].set_ylim(0.975 * np.min((np.nanmin(whole_wm['ihMTR'][is_measures]), np.nanmin(whole_wm_tilted['ihMTR'][is_measures]))),
                                      1.025 * np.max((np.nanmax(whole_wm['ihMTR'][is_measures]), np.nanmax(whole_wm_tilted['ihMTR'][is_measures]))))
            ax[row, col + 2].set_yticks([np.round(np.min((np.nanmin(whole_wm['ihMTR'][is_measures]), np.nanmin(whole_wm_tilted['ihMTR'][is_measures]))), decimals=1),
                                        np.round(np.max((np.nanmax(whole_wm['ihMTR'][is_measures]), np.nanmax(whole_wm_tilted['ihMTR'][is_measures]))), decimals=1)])
            ax[row, col + 2].set_xlim(0, 90)
            ax[row, col + 3].set_ylim(0.975 * np.min((np.nanmin(whole_wm['ihMTsat'][is_measures]), np.nanmin(whole_wm_tilted['ihMTsat'][is_measures]))),
                                      1.025 * np.max((np.nanmax(whole_wm['ihMTsat'][is_measures]), np.nanmax(whole_wm_tilted['ihMTsat'][is_measures]))))
            ax[row, col + 3].set_yticks([np.round(np.min((np.nanmin(whole_wm['ihMTsat'][is_measures]), np.nanmin(whole_wm_tilted['ihMTsat'][is_measures]))), decimals=1),
            ...
            axt4.scatter(whole_mid_bins[is_measures],
                        ...
                        c=whole_wm_tilted['Nb_voxels'][is_measures],
                        cmap='Greys', norm=norm,
                        edgecolors=cm.naviaS(8), linewidths=1, label="Tilted")
            ...
            ax[row, col + 2].axhline(y=mean_ihmtr, color=cm.naviaS(8), linewidth=1, alpha=0.5, linestyle="-")
            ...
            ax[row, col + 3].axhline(y=mean_ihmtsat, color=cm.naviaS(8), linewidth=1, alpha=0.5, linestyle="-")

            ax[row, col + 3].legend(loc=1)
