...
def main():
    ...
    out_path1 = out_folder / str("all_bundles_original_1f_MT.png")
    out_path2 = out_folder / str("all_bundles_original_1f_ihMT.png")
    ...
    for i in range(nb_bundles):
        ...
        if bundles_names[i] in extracted_bundles:
            ...
            ax[row, col].plot(highres_bins, BSpline(*ihmtr_fit)(highres_bins), "--", color=cm.naviaS(4))
            ...
            ax[row, col + 0].set_ylim(0.975 * np.nanmin(result['ihMTR']),
                                      1.025 * np.nanmax(result['ihMTR']))
            ax[row, col + 0].set_yticks([np.round(np.nanmin(result['ihMTR']), decimals=1),
                                         np.round(np.nanmax(result['ihMTR']), decimals=1)])
            ax[row, col + 0].set_xlim(0, 90)
            ax[row, col + 1].set_ylim(0.975 * np.nanmin(result['ihMTsat']),
                                      1.025 * np.nanmax(result['ihMTsat']))
            ax[row, col + 1].set_yticks([np.round(np.nanmin(result['ihMTsat']), decimals=1),
            ...
            ax[row, col + 0].plot(highres_bins, BSpline(*ihmtr_fit)(highres_bins), "--", color=cm.naviaS(4))
            ihmtsat_fit = splrep(whole_mid_bins[is_not_nan], whole_wm['ihMTsat'][is_not_nan], w=weights, s=0.0000005)
            ax[row, col + 1].plot(highres_bins, BSpline(*ihmtsat_fit)(highres_bins), "--", color=cm.naviaS(5))

            ax[row, col + 0].set_ylim(0.975 * np.nanmin(whole_wm['ihMTR']),
                                      1.025 * np.nanmax(whole_wm['ihMTR']))
            ax[row, col + 0].set_yticks([np.round(np.nanmin(whole_wm['ihMTR']), decimals=1),
                                         np.round(np.nanmax(whole_wm['ihMTR']), decimals=1)])
            ax[row, col + 0].set_xlim(0, 90)
            ax[row, col + 1].set_ylim(0.975 * np.nanmin(whole_wm['ihMTsat']),
                                      1.025 * np.nanmax(whole_wm['ihMTsat']))
            ax[row, col + 1].set_yticks([np.round(np.nanmin(whole_wm['ihMTsat']), decimals=1),
