...
def main():
    ...
    fig, ax = plt.subplots(2, 2, subplot_kw=dict(projection='3d'), layout='constrained')
    X, Y = np.meshgrid(mid_bins, mid_bins)
    ax[0, 0].plot_surface(X, Y, mtr[0], cmap=cm.navia)
    ax[0, 0].set_zlabel('MTR')
    ax[0, 1].plot_surface(X, Y, ihmtr[0], cmap=cm.navia)
    ax[0, 1].set_zlabel('ihMTR')
    ax[1, 0].plot_surface(X, Y, mtsat[0], cmap=cm.navia)
    ax[1, 0].set_zlabel('MTsat')
    ax[1, 1].plot_surface(X, Y, ihmtsat[0], cmap=cm.navia)
